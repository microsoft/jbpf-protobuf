...
#include <pb_decode.h>
#include <pb_encode.h>
#include "{{ .ProtoPackageName }}.pb.h"

const uint32_t proto_message_size = sizeof({{ .ProtoMessageName }});
...
int jbpf_io_serialize(void* input_msg_buf, size_t input_msg_buf_size, char* serialized_data_buf, size_t serialized_data_buf_size) {
	...
	if (!pb_encode(&ostream, {{ .ProtoMessageName }}_fields, input_msg_buf))
		return -1;
...
int jbpf_io_deserialize(char* serialized_data_buf, size_t serialized_data_buf_size, void* output_msg_buf, size_t output_msg_buf_size) {
	...
	return pb_decode(&istream, {{ .ProtoMessageName }}_fields, output_msg_buf);
}
